...
def apply_random_transformations_and_save(img, train_output_folder, test_output_folder, station_name, num_new_images=100, grayscale=True):
    for i in range(num_new_images):
        ...
        # Decide if it's train
        output_folder = train_output_folder if i < int(num_new_images * 0.8) else test_output_folder

        filename = f"{station_name}_{i}.jpg"

        # Save image in normal position with transformations
...
def augment_dataset(input_folder, output_base_folder, grayscale=True):
    if grayscale:
        output_base_folder += '/g_scale'
    else:
        output_base_folder += '/color'
